    ...
    <div class="container">
        ...
        <div class="mt-4">
            <div class="header-container rounded text-center mb-4">
                <h1 class=" font-weight-bold text-primary">Pemeriksaan</h1>
            </div>
            ...
            <form action="{{ route('admin.pemeriksaan') }}" method="GET" class="mb-4 fade-in-left">
                <!-- Form pencarian -->
                <div class="row">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        <div class="input-group mb-2">
                            ...
                            <div class="input-group-append">
                                <a href="{{ route('admin.pemeriksaan') }}" class="btn btn-secondary">Reset</a>
                            </div>
                        </div>
                    </div>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn text-warning btn-sm mr-2"
                                        ...
                                        data-bs-target="#editPemeriksaanModal{{ $periksa->id_periksa }}"><i
                                            class="fa-solid fa-pen"></i></button>
                                    <form action="{{ route('admin.pemeriksaan.destroy', $periksa->id_periksa) }}" method="POST"
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.pemeriksaan.update', $periksa->id_periksa) }}" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="payment" class="form-label">Payment:</label>
                            <select class="form-select" id="payment" name="payment" required>
                                <option value="0" {{ $periksa->payment == 0 ? 'selected' : '' }}>0</option>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="payment_status" name="payment_status" required>
                                <option value="lunas" {{ $periksa->payment_status == 'lunas' ? 'selected' : '' }}>LUNAS</option>
                                <option value="belum-lunas" {{ $periksa->payment_status == 'belum-lunas' ? 'selected' : '' }}>BELUM-LUNAS</option>
                            </select>
                        </div>
                        @foreach (['WBC', 'RBC', 'PLT', 'HGB', 'HTM', 'Neu', 'Eos', 'Bas', 'Lym', 'Mon', 'MCV', 'MCH', 'MCHC'] as $field)
                            <div class="mb-3">
